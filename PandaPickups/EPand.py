...
class ColTime(DirectObject.DirectObject):
    def __init__(self):
        ...
        self.simco.addSolid(CollisionSphere(self.anim.node().getBounds().getCenter(),self.anim.node().getBounds().getRadius()*1.2))
...
class P3(DirectObject.DirectObject):
    ...
    def moving(self):
        ...
        if len(entries) > 0:
            ...
            self.P1.modactIn.setTransparency(TransparencyAttrib.MAlpha)
            self.P1.modactIn.setAlphaScale(0.2)
...
bop=P3(hifi)
care=ColTime()
care.anim.setPos(0,0,30)
#base.cTravA.addCollider(hifi.modcollision,care.farnve)
WorldA.run()
